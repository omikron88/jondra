...
public final class Keyboard implements KeyListener {
    ...
    private boolean isEnabled = true;  // Výchozí stav: klávesnice je aktivní
    ...
    public void setKeyboardEnabled(boolean enabled) {
        isEnabled = enabled;
        if (!enabled) {
            clearKeyboardBuffer(); // Pokud klávesnici vypínáme, vyprázdníme buffer
        }
    }

    public void clearKeyboardBuffer() {
        for (int i = 0; i < io.length; i++) {
            io[i] = (byte) 0xff;  // Resetuje klávesy do výchozího stavu
        }
    }
    ...
    public void keyPressed(KeyEvent ke) {
        if (!isEnabled) {
            return;  // Pokud je klávesnice vypnutá, ignoruj vstupy
        }
        bOndraKey = false;
        ...
        if (bOndraKey) {
            processKeyboardPicturePress(ke);
        }
    }
